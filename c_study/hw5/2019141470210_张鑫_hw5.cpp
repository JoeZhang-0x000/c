...
void main()
{
	...
	while (i < 80)
	{
		...
		//寻找B
		if (iuserth == imagb||iuserth == imagc||iuserth == imagd)
		{
			B++;
		}
		if (iuserh == imaga||iuserh == imagc||iuserh == imagd)
		{
			B++;
		}
		if (iusert == imaga||iusert == imagb||iusert == imagd)
		{
			B++;
		}
		if (iusero == imaga||iusero == imagb||iusero == imagc)
		{
			B++;
		}
		//打印结果
...
//获取百位数字
int getthundred(int inum)
{
	return (inum - inum / 1000 * 1000) / 100;
}
//获取十位数字
int getten(int inum)
{
